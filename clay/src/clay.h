            ...
            if (clipHashMapItem && !Clay__ElementIsOffscreen(&clipHashMapItem->boundingBox)) {
                ...
                if (generateRenderCommands) {
                    Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand) {
                         ...
                         * from the tree containing its inherited clip element.
                         * Clay therefore emits this synthetic scissor pair
                         * around the floating root. Previously the start command
                         * copied the clip bounding box but left Clay_ClipRenderData
                         * zero-initialized.
                         *
                         * Renderers that always apply a rectangular hardware
                         * scissor do not observe the missing flags. The Defold GUI
                         * backend maps Clay scissors to stencil nodes, however,
                         * and uses the flags to emulate a one-axis clip by
                         * expanding the disabled axis to the layout bounds. A
                         * zero/zero command was consequently expanded on both
                         * axes, allowing the floating Clay debug hierarchy to
                         * render over its configuration footer.
                         *
                         * Copy the inherited clip element's axis configuration so
                         * synthetic commands have the same semantics as ordinary
                         * clip-element commands. This also preserves horizontal-
                         * only and vertical-only clipping for user floating roots.
                         */
                        .renderData = { .clip = {
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (generateRenderCommands && !offscreen) {
                if (currentElement->isTextElement) {
                     ...
                     * and image data therefore emitted these commands, in order:
                     *
                     *   1. CLAY_RENDER_COMMAND_TYPE_IMAGE
                     *   2. CLAY_RENDER_COMMAND_TYPE_RECTANGLE
                     *
                     * Both commands had the same element id, bounding box,
                     * background color, and corner radii. A renderer that uses
                     * `Clay_ImageRenderData.backgroundColor` as the image tint
                     * draws the image first and then covers it with the opaque
                     * rectangle. Transparent pixels used for rounded image
                     * corners are consequently hidden.
                     *
                     * Minimal triggering declaration:
                     *
                     *   CLAY(CLAY_ID("Image"), {
                     ...
                     *   }) {}
                     *
                     * The public declaration comment says backgroundColor
                     * generates a RECTANGLE only when no other visual config is
                     * specified; otherwise it is passed to IMAGE or CUSTOM.
                     * The README's Clay_ImageElementConfig rendering section
                     * likewise says an image element creates a single IMAGE
                     * command and leaves blending of
                     * `renderData.image.backgroundColor` to the renderer.
                     *
                     * Excluding IMAGE and CUSTOM elements from this RECTANGLE
                     * branch restores that documented behavior:
                     *
                     *   - background only: one RECTANGLE
                     *   - image + background: one tinted IMAGE
                     *   - custom + background: one tinted CUSTOM
                     *
                     * This condition should be compared with upstream when
                     * updating clay.h and removed once upstream has an
                     * equivalent fix.
                     */
